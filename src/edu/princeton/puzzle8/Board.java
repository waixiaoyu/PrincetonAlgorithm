...
public class Board {
	...
	public Board twin() {
		...
		if (b.nAABlocks[0][0] == 0) {
			swap(2, 3, b.nAABlocks);
		} else {
			if (b.nAABlocks[0][1] == 0) {
				swap(1, 3, b.nAABlocks);
			} else {
				swap(1, 2, b.nAABlocks);
			}
		}
		return b;
	}
	...
	private void swap(int index1, int index2, int[][] aa) {
		...
		swap(co1[0], co1[1], co2[0], co2[1], aa);
	}

	private void swap(int x1, int y1, int x2, int y2, int[][] aa) {
		int temp = aa[x1][y1];
		aa[x1][y1] = aa[x2][y2];
		aa[x2][y2] = temp;
	}
	...
	private void swap(int x1, int y1, int x2, int y2) {
		swap(x1, y1, x2, y2, this.nAABlocks);
	}
